---
source_url: "https://www.palantir.com/docs/foundry/api/llm-apis/models/openai-responses-proxy/"
title: "OpenAI Responses (Proxy) \u2022 API Reference"
---
```
POST /api/v2/llm/proxy/openai/v1/responses
```

Foundry provides a proxy endpoint that forwards requests to the [OpenAI Responses API ↗](https://developers.openai.com/api/reference/resources/responses/methods/create). This enables the use of open-source SDKs and tooling while benefiting from Foundry capabilities such as rate limiting, data governance, and usage tracking.
...
This endpoint has the same shape as the OpenAI Responses API. Refer to the [OpenAI Responses API documentation ↗](https://developers.openai.com/api/reference/resources/responses/methods/create) for the full schema.
...
curl -X POST \
    ...
    "https://$HOSTNAME/api/v2/llm/proxy/openai/v1/responses" \
    -d '{
...
export default async function openAiResponses(
