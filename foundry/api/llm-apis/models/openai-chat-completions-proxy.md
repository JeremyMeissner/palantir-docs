---
source_url: "https://www.palantir.com/docs/foundry/api/llm-apis/models/openai-chat-completions-proxy/"
title: "OpenAI Chat Completions (Proxy) \u2022 API Reference"
---
```
POST /api/v2/llm/proxy/openai/v1/chat/completions
```

Foundry provides a proxy endpoint that forwards requests to the [OpenAI Chat Completions API ↗](https://developers.openai.com/api/reference/resources/chat/subresources/completions/methods/create). This enables the use of open-source SDKs and tooling, while benefiting from Foundry capabilities such as rate limiting, data governance, and usage tracking.
...
This endpoint has the same shape as the OpenAI Chat Completions API. Refer to the [OpenAI Chat Completions API documentation ↗](https://developers.openai.com/api/reference/resources/chat/subresources/completions/methods/create) for the full schema.
...
curl -X POST \
    ...
    "https://$HOSTNAME/api/v2/llm/proxy/openai/v1/chat/completions" \
    -d '{
      "model": "ri.language-model-service..language-model.gpt-5-2",
      "messages": [
...
def openai_chat_completions(prompt: str) -> str:
    ...
    completion = client.chat.completions.create(model="ri.language-model-service..language-model.gpt-5-2", messages=[{"role": "user", "content": prompt}])
