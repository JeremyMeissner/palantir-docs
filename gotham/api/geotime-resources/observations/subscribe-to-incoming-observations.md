---
source_url: "https://www.palantir.com/docs/gotham/api/geotime-resources/observations/subscribe-to-incoming-observations/"
title: "Subscribe to incoming observations \u2022 API Reference"
---
...
```sh
GET "wss://$HOSTNAME/api/gotham/ws/v1/observations/$OBSERVATION_SPEC_ID?preview=true" \
...
Once the connection is established, you must send a `subscribe` message to inform the server how to filter to the data you're interested in receiving. The query language supports the same filtering capability as the [search observations endpoint](/docs/gotham/api/geotime-resources/observations/search-latest-observations/).
...
{
  "type": "subscribed",
  "id": "3255ebeb-a897-437c-ae7b-99c5d64d17cf",
  "errors": []
}
```
...
```yaml
Error:
  error: string # a name for the error
  args: array<Arg> # parameters helping describe the failure, for example, enumerating invalid inputs
...
Once you are subscribed, the server will begin forwarding any newly-ingested observation data matching your subscription via `data` messages (see [Receiving data](#receiving-data))
...
```yaml
id: uuid # the subscription ID this data relates to
observations: array<Observation> # the data
```
  ...
  "observations": [
    {
      ...
      "trackId": "track0",
      "position": {
        "longitude": -77.05974824713265,
        "latitude": 38.903335277742656
      },
...
```python
# NOTE: you must install websockets module
...
OBSERVATION_SPEC_ID = <your_observation_spec_id>
TOKEN = <your_token>

endpoint = f"wss://{HOSTNAME}/api/gotham/ws/v1/observations/{OBSERVATION_SPEC_ID}?preview=true"
websocket_token=f"Bearer-{TOKEN}"

async def connect_and_listen():
    async with websockets.connect(endpoint, subprotocols=[websocket_token], max_size=2**31, read_limit=2**31) as websocket:
        print("Websocket opened")
        
        subscribe = '{"type": "subscribe", "query": {}}}'
...
async def main():
    await asyncio.create_task(connect_and_listen())
    print("Terminated")
