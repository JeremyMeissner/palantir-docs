---
source_url: "https://www.palantir.com/docs/gotham/api/general/overview/errors/"
...
be returned with a `4xx` or `5xx` status code. For example, a request to load 
an object with primaryKey `invalidkey` might look like the following:
...
In exceptional circumstances, the service may return other, undocumented errors. Note that all errors returned by this API are compliant and consistent with [HTTP status code semantics](https://www.rfc-editor.org/rfc/rfc9110.html#name-status-codes). Contact your system administrator to resolve these errors.
:::
